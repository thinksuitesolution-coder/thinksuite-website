  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const triggers = Array.from(cards).map((card) => {
        ...
        gsap.set([num, title, desc].filter(Boolean), { opacity: 0, y: 32 })

        const tl = gsap.timeline({ paused: true })
        tl.to(num,   { opacity: 1, y: 0, duration: 0.5,  ease: 'power3.out' })
          .to(title, { opacity: 1, y: 0, duration: 0.65, ease: 'power3.out' }, '-=0.25')
          .to(desc,  { opacity: 1, y: 0, duration: 0.55, ease: 'power3.out' }, '-=0.3')
