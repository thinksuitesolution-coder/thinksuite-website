...
export async function POST(req) {
  try {
    ...
    const { toolSlug, toolName, featureId, featureName, message, rating } = body;
    if (!toolSlug || !message?.trim()) {
      return NextResponse.json({ error: "Tool and message are required" }, { status: 400 });
    }
