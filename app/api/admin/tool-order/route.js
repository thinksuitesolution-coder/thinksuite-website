...
export async function POST(req) {
  try {
    const { idToken, toolOrder, featureOrders } = await req.json();
    if (!idToken) return NextResponse.json({ error: "idToken required" }, { status: 401 });
    const { adminApp } = await verifyAdmin(idToken);

    const update = {};
    if (toolOrder !== undefined) update.toolOrder = toolOrder;
    if (featureOrders !== undefined) update.featureOrders = featureOrders;

    await adminApp.firestore().doc("config/toolOrder").set(update, { merge: true });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[admin/tool-order]", err.message);
